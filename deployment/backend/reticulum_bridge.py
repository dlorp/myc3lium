...
class ATAKHandler:
    ...
    def create_cot_xml(self, event: CoTEvent) -> str:
        ...
        stale = datetime.fromtimestamp(
            now.timestamp() + 300, timezone.utc
        )  # 5 min stale
...
class ReticulumManager:
    ...
    def _init_lxmf(self):
        ...
        self.lxmf_router = LXMF.LXMRouter(
            identity=self.identity, storagepath=CONFIG["lxmf_storage"]
        )
    ...
    def get_mesh_stats(self) -> Dict:
        ...
        stats = {
            "transport": {
                ...
                "destination_table_size": len(Transport.destination_table)
...
async def mesh_stats():
    ...
    return {
        "reticulum": rns_stats,
        "meshtastic": {"node_count": len(meshtastic_nodes), "nodes": meshtastic_nodes},
        "total_nodes": len(rns_manager.get_known_destinations())
        + len(meshtastic_nodes),
    }
...
async def mesh_destinations():
    ...
    return {"destinations": rns_dests + mesh_dests}
...
async def get_messages(limit: int = 100):
    ...
    return {"messages": message_history[-limit:], "total": len(message_history)}
...
async def send_position(lat: float, lon: float, callsign: str):
    ...
    await broadcast_to_websockets(
        {"type": "cot_event", "data": {"lat": lat, "lon": lon, "callsign": callsign}}
    )
...
async def get_cameras():
    ...
    return {"streams": camera_manager.get_streams()}
