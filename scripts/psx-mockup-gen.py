...
def panel_split(
    width: int, height: int, left_title: str, right_title: str, split_ratio: float = 0.5
) -> List[str]:
    ...
    top = (
        f"{c_blue}{BOX['tl']}{BOX['h'] * left_left}{left_title_pad}{BOX['h'] * left_right}"
    ...
    for _ in range(height - 2):
        lines.append(
            f"{c_blue}{BOX['v']}{r}{' ' * left_width}{c_blue}{BOX['v']}{r}{' ' * right_width}{c_blue}{BOX['v']}{r}"
        )
...
def demo_dashboard():
    ...
    for i, line in enumerate(layout):
        if i == 0 or i == len(layout) - 1:
            ...
            row = i - 1
            left = (
                f"  {left_content[row]:<30}"
                if row < len(left_content)
                else "  " + " " * 30
            )
            right = (
                f"  {right_content[row]:<38}"
                if row < len(right_content)
                else "  " + " " * 38
            )
